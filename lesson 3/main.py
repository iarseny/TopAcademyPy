...
def purchase(user_id):
    ...
    if product:
        ...
        if quantity >= 0 and balance >= price:
...
def admin_login():
    ...
    c.execute(f"SELECT * FROM admins WHERE username = ? AND password = ?", (username, password))
    admin = c.fetchone()
